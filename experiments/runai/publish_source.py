...
def main():
    ...
    if previous.get('source_commit')==head and previous.get('bundle_sha256')==manifest['bundle_sha256']:
        revision=parent
    else:
        ...
        result=api.create_commit(repo_id=REPO,parent_commit=parent,
            operations=[CommitOperationAdd(path_in_repo=x.name,path_or_fileobj=str(x)) for x in (bundle,mp)],
    ...
    for local in (bundle,mp):
        ...
        if local==mp:
            ...
            for key in ('source_commit','bundle_sha256','bundle_bytes','source_branch'):
