...
def main(a):
    ...
    cfg=dict(student=str(base/'SimCT/runs/qwen-gemma-sft-paper-20260908-045828/checkpoint'),teacher='/workspace/storage-shared/models/Qwen2.5-7B-Instruct',reference_input=str(a.references or base/'SimCT/data/qwen-author/data/selected.parquet'),reference_key=a.reference_key,exclude=a.exclude,eval_template=str(a.eval_template.resolve()),scope='external guide; provenance and near-duplicate audit still required' if a.references else 'SEEN-SFT mechanics only; not an unseen generalization gate')
    for key in ('student','teacher','reference_input','eval_template'):
    ...
    plan=dict(schema='campaign-v1',hours=20,source=str(ROOT),source_commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),jobs=jobs,data_contract=cfg)
