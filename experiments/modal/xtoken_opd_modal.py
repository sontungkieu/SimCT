"""Run one exact, evidence-gated 10-update X-Token OPD workload on Modal."""
...
APP_NAME = "vdt-xtoken-opd-no1ceboy-20260904-r7"
RUN_ID = "xtoken-opd-a10x2-10update-20260904-r7"
VOLUME_NAME = "vdt-xtoken-opd-no1ceboy-20260904-r7"
...
WANDB_RUN_ID = "xtoken-opd-r7-9d5c801"
WANDB_RUN_NAME = "xtoken-opd-modal-a10x2-10update-r7"

STUDENT_REVISION = "4e20de362430cd3b72f300e6b0f18e50e7166e08"
TEACHER_REVISION = "1cfa9a7208912126459214e8b04321603b3df60c"
PROJECTION_SHA256 = "9c9bac81ab0151404ad9030b3d52d9af35b14eddb1547b21461e82f4c9121cf3"
...
PROJECTION = TARGET_ROOT / "projection-full/runtime_top4.pt"
...
def scientific_contract() -> dict[str, object]:
    return {
        ...
        "algorithm": "NVIDIA X-Token P-KL+CE adapted to KDFlow OPD",
...
def train() -> dict[str, object]:
    ...
    try:
        ...
        for required in (STUDENT / "config.json", TEACHER / "config.json", PROJECTION):
            if not required.is_file():
                raise FileNotFoundError(str(required))
        if sha256(PROJECTION) != PROJECTION_SHA256:
            raise RuntimeError("projection SHA mismatch")
        ...
        if unit.returncode:
            raise RuntimeError(f"X-Token unit test failed with exit code {unit.returncode}")
        ...
        command = [
            ...
            "--kd_loss_fn", "kl",
            "--kd_temperature", "1.0",
            "--xtoken_projection_path", str(PROJECTION),
            "--xtoken_projection_sha256", PROJECTION_SHA256,
            "--xtoken_vocab_topk", "8192",
            "--xtoken_dynamic_loss_scaling", "True",
            "--xtoken_exact_match_only", "False",
            ...
            "--wandb_tags", "opd,xtoken,modal,a10x2,10-update,llama-3.2-1b,qwen3-4b",
            "--wandb_mode", "online",
