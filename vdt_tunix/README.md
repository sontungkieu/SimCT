...
| Real frozen-teacher scoring | implemented, cached retry pending | exact completion retokenization; frozen Qwen compute is explicitly BF16 before model construction, the fused dense reference reduces full-vocabulary causal rows inside the JIT, and the 4K/8K resource ladders use parity-tested cached teacher forcing with the same compute dtype for KV storage |
