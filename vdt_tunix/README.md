...
| Real frozen-teacher scoring | implemented, masked-max cached retry pending | exact completion retokenization; frozen Qwen compute is explicitly BF16 before model construction, the fused dense reference reduces full-vocabulary causal rows inside the JIT, and cached teacher forcing carries only hidden state while barrier-bracketed one-step LM-head projections are immediately reduced to exact sufficient statistics through a masked vocabulary maximum |
