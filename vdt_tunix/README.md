...
| Real frozen-teacher scoring | implemented, execution pending | exact completion retokenization; frozen Qwen compute is explicitly BF16 before model construction, the fused forward reduces full-vocabulary causal rows inside the JIT, and cached teacher forcing uses the same compute dtype for KV storage |
