...
| Real frozen-teacher scoring | implemented, execution pending | exact completion retokenization; the production fused forward reduces full-vocabulary causal rows inside the JIT to shared-token and realized selected-token log-probabilities before returning them; cached teacher forcing uses the pinned model compute dtype for KV storage |
