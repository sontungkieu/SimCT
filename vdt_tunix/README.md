...
| Real frozen-teacher scoring | implemented, blocked-normalizer cached retry pending | exact completion retokenization; frozen Qwen compute is explicitly BF16 before model construction, and cached teacher forcing carries only hidden state while barrier-bracketed one-step projections are immediately reduced to exact sufficient statistics; realized/shared logits use immutable LM-head rows, while the exact full-vocabulary log-normalizer is reduced in static blocks and composed with stable `logaddexp` |
